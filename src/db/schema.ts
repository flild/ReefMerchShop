...
export const materials = sqliteTable("materials", {
  ...
  typeId: text("type_id")
    .notNull()
    .references(() => materialTypes.id, { onDelete: "restrict" }), // Привязка к типу
  name: text("name").notNull(),
  description: text("description"),
...
export const collectParticipants = sqliteTable("collect_participants", {
  id: text("id").primaryKey(),
  collectId: text("collect_id")
    .notNull()
    .references(() => collects.id, { onDelete: "cascade" }),
  ...
  isLayoutsUploaded: integer("is_layouts_uploaded", { mode: "boolean" })
    .notNull()
    .default(false),
  status: text("status").notNull().default("new"),
  createdAt: timestampMs("created_at"),
...
export const checklistBlocks = sqliteTable("checklist_blocks", {
  id: text("id").primaryKey(),
  templateId: text("template_id")
    .notNull()
    .references(() => checklistTemplates.id, { onDelete: "cascade" }),
  type: text("type").notNull(), // 'text', 'select', 'checkbox', 'image', 'number', 'custom'
  title: text("title").notNull(),
  description: text("description"),
  isRequired: integer("is_required", { mode: "boolean" })
    .notNull()
    .default(false),
  optionsJson: text("options_json").notNull().default("[]"), // [{ label: 'Option 1', imageUrl: '...' }]
...
export const checklistTemplatesRelations = relations(
  checklistTemplates,
  ({ many }) => ({
    blocks: many(checklistBlocks),
  }),
);

export const checklistBlocksRelations = relations(
  checklistBlocks,
  ({ one }) => ({
    template: one(checklistTemplates, {
      fields: [checklistBlocks.templateId],
